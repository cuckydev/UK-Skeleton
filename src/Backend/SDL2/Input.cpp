/*
 * UK Skeleton
 * File: Backend/SDL2/Input.cpp
 * 
 * Purpose: Define SDL2 input interface
 * 
...
namespace Backend
{
	namespace Input
	{
		...
		static bool input_state[InputCode_Num];
		
		//Input interface
		/*
		 * Backend::Input::Initialize:
		 *   Initializes the input backend subsystem
		 * 
		 * Returns:
		...
		*/
		...
		/*
		 * Backend::Input::Quit:
		 *   Deinitializes the input backend subsystem
		 * 
		 * Returns:
		 *   Status code (false on success, true on failure)
		*/
		...
		/*
		 * Backend::Input::HandleEvents:
		 *   Handles all events sent from SDL2
		 * 
		 * Returns:
		 *   If SDL_QUIT was recieved (end the program)
		*/
		bool HandleEvents()
		{
			SDL_Event event;
			while (SDL_PollEvent(&event))
			{
				switch (event.type)
				{
					...
					case SDL_KEYDOWN:
					case SDL_KEYUP:
						input_state[map_scancode_inputcode[event.key.keysym.scancode]] = event.key.state == SDL_PRESSED;
						break;
				}
			}
			return false;
		...
		/*
		 * Backend::Input::IsInputDown:
		 *   Checks if the given input code is held down
		 * 
		 * Arguments:
		 *   code - Input code to check
		 * 
		 * Returns:
		 *   If the given input code is held down
		*/
